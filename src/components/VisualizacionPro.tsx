...
import { Settings, AlertCircle, AlertTriangle, Bell, Check, X, History, Printer, ChevronLeft, ChevronRight } from 'lucide-react';
...
const VisualizacionPro: React.FC<DashboardProps> = ({ data, data2h, allData, latestPoint, loading, meta, timeRange, onTimeRangeChange, isHistorical }) => {
    ...
    <div className="flex flex-col gap-2 h-full w-full overflow-hidden select-none">
      ...
      {/* ═══ TOP ROW: Metrics Bar (Full Width) ═══ */}
         <div className="flex gap-2 shrink-0 h-[125px]">
               ...
               {/* Weather Widget */}
               <div className="w-[175px] shrink-0 glass-panel flex flex-col bg-[#1c1c1e] relative overflow-hidden border border-white/5 px-2 py-2">
                  {/* Top: Today's weather */}
              ...
              {/* Nivel de Gases (moved from bottom row) */}
              <div className="w-[115px] shrink-0 glass-panel flex flex-col items-center pt-2 pb-1 bg-[#1D1D20] relative">
                  ...
                  <div className="flex-1 flex flex-row w-full overflow-hidden">
                     ...
                     <div className="flex flex-col border-r-2 border-[#525252] flex-1">
                        ...
                        <div className="flex-1 flex flex-row">
                           {/* RIH — celda izquierda, altura completa */}
                           ...
                           {/* Juntas/hora + Conexión-Desconexión — apiladas a la derecha */}
      ...
      <div className="flex gap-2 flex-1 min-h-0 w-full">
       ...
       <div className="flex flex-[2.2] flex-col gap-2 min-w-0 h-full">
         <div className="flex-1 min-h-0 glass-panel overflow-hidden p-0 relative">
            <WellChart data={data} latestPoint={latestPoint} loading={loading} timeWindow={timeRange} onTimeWindowChange={onTimeRangeChange} isHistorical={isHistorical} onShowHistory={() => setShowHistoryModal(true)} showBrushPanel={true} showUnitSelector={true} />
         </div>
       </div>
       ...
       <div className="flex flex-col flex-1 gap-2 h-full min-w-0">
         ...
         <div className="grid grid-cols-[1.4fr_1fr] gap-2 h-[220px] shrink-0">
            <div className="glass-panel p-3 flex flex-col">
               ...
               <div className="flex flex-col gap-2 flex-1 justify-center">
                  {/* Alerta roja: torque fuera de rango */}
                  <div className="flex items-center gap-2 rounded-md px-2 py-2" style={{ background: 'rgba(160,25,15,0.90)' }}>
                     ...
                     <span className="flex-1 text-[10px] font-bold text-white leading-tight">
                        Torque fuera de rango: último valor 11666 (KPI 4300 - 5100)
                     </span>
                  ...
                  {/* Alerta amarilla: juntas por debajo del KPI */}
                  <div className="flex items-center gap-2 rounded-md px-2 py-2" style={{ background: 'rgba(140,105,0,0.90)' }}>
                     ...
                     <span className="flex-1 text-[10px] font-bold text-white leading-tight">
                        Juntas por hora: último registro 8.78 está por debajo del KPI (18)
                     </span>
         ...
         {/* Bottom Row Layout: Tonelada Milla + Box 15 */}
         <div className="flex gap-2 h-[130px] shrink-0 mb-0 w-full">
            {/* Box 6: Tonelada Milla — solo este en la izquierda */}
            <div className="w-[120px] shrink-0 h-full">
               <div className="glass-panel h-full flex flex-col bg-[#1D1D20] relative overflow-hidden pl-2 pr-2 pt-2 pb-1">
                  {/* Winch SVG Background */}
                  <div className="absolute top-[0px] bottom-0 -right-2 w-[70px] flex items-center justify-center opacity-70 pointer-events-none">
                  ...
                  <div className="relative z-10 w-full flex flex-col h-full pt-1">
                     ...
                     <div className="flex-1 flex flex-col justify-center items-start pl-2 text-left">
                        <span className="text-[22px] font-black text-white leading-none drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">181.98</span>
                     </div>
                  </div>
               </div>
            </div>
      ...
      {/* ═══ HISTORICAL VIEW — card flotante sobre fondo #32343E ═══ */}
      {showHistoryModal && (
        <div
          className="fixed left-0 right-0 bottom-0 z-[9999] flex p-3 select-none"
          style={{ top: '50px', background: '#32343E' }}
        >
          <style>{`
          ...
          <div
            ...
            style={{
              ...
              animation: 'expandCard 0.22s cubic-bezier(0.2,0,0,1)',
          ...
          >
            ...
            <div className="h-[40px] shrink-0 flex items-center justify-between px-4 border-b border-[#525252]" style={{ background: '#1F1F1F' }}>
              <button
                onClick={() => { setShowHistoryModal(false); setIsExportOpen(false); }}
                className="flex items-center gap-1.5 text-white/45 hover:text-white transition-colors group"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/15 text-[10px]">·</span>
                <span className="text-[10px] font-bold text-white/40 uppercase tracking-wider">WORKOVER</span>
                <span className="text-white/15 text-[10px]">·</span>
            ...
            <div className="flex-1 relative overflow-hidden">
              ...
              {/* Botón impresora flotante */}
              <button
                onClick={() => setIsExportOpen(!isExportOpen)}
                className="absolute top-1/2 -translate-y-1/2 z-[160] w-[36px] h-[44px] bg-[#F0F0F0] text-[#121214] rounded-l-full shadow-2xl flex items-center justify-center hover:bg-white transition-all duration-300 ease-in-out"
                style={{ right: isExportOpen ? '400px' : '0' }}
              >
                {isExportOpen
                  ? <ChevronRight size={16} className="opacity-80" />
                  : <Printer size={18} strokeWidth={2.5} className="mr-0.5 opacity-90" />
                }
              </button>
...
export default VisualizacionPro;
