...
import { Settings, AlertTriangle, Bell, Check, X, History, Printer, ChevronLeft, ChevronRight, ChevronDown } from 'lucide-react';
...
const VisualizacionPro2: React.FC<DashboardProps> = ({ data, data2h, allData, latestPoint, loading, timeRange, onTimeRangeChange, isHistorical }) => {
  ...
  return (
    <div className="flex flex-col gap-2 h-full w-full overflow-hidden select-none">
      ...
      <style>{`
        @keyframes histExportCursor {
          0%, 15% { transform: translate(100px, 130px); opacity: 0; }
          20% { transform: translate(100px, 130px); opacity: 1; }
          30% { transform: translate(280px, 92px); opacity: 1; }
          35% { transform: translate(280px, 92px) scale(0.8); opacity: 1; }
          38% { transform: translate(280px, 92px) scale(1); opacity: 1; }
          45% { transform: translate(280px, 92px); opacity: 1; }
          55% { transform: translate(280px, 92px); opacity: 0; }
          100% { opacity: 0; }
        }
        ...
        @keyframes histExportBtnClick {
          0%, 34%, 39%, 100% { transform: scale(1); background: #00809D; }
          35%, 38% { transform: scale(0.95); background: #00A3C4; }
        }
        .demo-cursor {
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={currentBarData8} margin={{ top: 4, right: 2, left: 0, bottom: 16 }}>
                  ...
                  <Bar dataKey="val" radius={[1,1,0,0]} maxBarSize={12}>
                    {currentBarData8.map((_,i) => <Cell key={i} fill={chartType8 === 'juntas' ? '#8b5cf6' : (i%2===0 ? 'url(#gradientSencillo2)' : 'url(#gradientDoble2)')} />)}
                  </Bar>
          ...
          <div className="flex-1 min-h-0 glass-panel overflow-visible p-0 relative">
            <WellChart
              ...
              historyPreviewContent={
                <div className="absolute top-[-200px] left-[110px] w-[800px] h-[480px] border border-[#00809D]/50 rounded-xl shadow-[0_30px_100px_rgba(0,0,0,1)] p-4 z-[99999] pointer-events-none flex flex-col origin-left animate-in fade-in zoom-in duration-300" style={{ backgroundColor: '#1F1F1F' }}>
                   <style>{`
                      @keyframes histCursor {
                        0%, 5% { transform: translate(500px, 300px); opacity: 0; }
                        8% { transform: translate(500px, 300px); opacity: 1; }
                        15% { transform: translate(140px, 38px); opacity: 1; }
                        25% { transform: translate(140px, 38px); opacity: 1; }
                        30% { transform: translate(400px, 135px); opacity: 1; }
                        32% { transform: translate(400px, 135px) scale(0.8); opacity: 1; }
                        34% { transform: translate(400px, 135px) scale(1); opacity: 1; }
                        38% { transform: translate(400px, 135px); opacity: 1; }
                        45% { transform: translate(840px, 130px); opacity: 1; }
                        48% { transform: translate(840px, 130px) scale(0.8); opacity: 1; }
                        55% { transform: translate(840px, 200px) scale(0.8); opacity: 1; }
                        57% { transform: translate(840px, 200px) scale(1); opacity: 1; }
                        65% { transform: translate(950px, 250px); opacity: 1; }
                        80% { transform: translate(1000px, 450px); opacity: 0; }
                        100% { opacity: 0; }
                      }
                   ...
                   <div className="flex items-center justify-between mb-3 shrink-0">
                      ...
                      <div className="flex items-center gap-2 border border-white/10 px-3 py-1.5 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
                         <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: '#00809D' }}></div>
                         <span className="text-[9px] font-bold text-white uppercase tracking-wider">Animación Activa</span>
                      </div>
                   </div>

                   {/* Sliding Container */}
                   <div className="flex-1 w-full rounded-lg border border-white/10 overflow-hidden relative shadow-inner" style={{ backgroundColor: '#111111' }}>
                      <div className="flex w-[200%] h-full" style={{ animation: 'histScreenSlide 10s cubic-bezier(0.8, 0, 0.2, 1) infinite' }}>
                         
                         {/* SCREEN 1: Table List */}
                         <div className="w-1/2 h-full flex flex-col p-6 relative" style={{ backgroundColor: '#1A1A1A' }}>
                            {/* Fake Cursor */}
                            ...
                            <div className="flex justify-between items-center mb-6">
                               <div className="flex flex-col gap-1 w-[200px]">
                                  ...
                                  <div className="w-full h-[32px] border border-white/20 rounded px-3 flex items-center text-[11px] text-white font-bold relative" style={{ backgroundColor: '#111111' }}>
                            ...
                            <div className="flex-1 border border-white/5 rounded-t-lg overflow-hidden" style={{ backgroundColor: '#222222' }}>
                               <table className="w-full text-left text-[11px] text-white/80">
                                  <thead className="text-white border-b border-[#00809D]/50" style={{ backgroundColor: '#333333' }}>
                                     <tr>
                         ...
                         <div className="w-1/2 h-full relative overflow-hidden flex flex-col" style={{ backgroundColor: '#111111' }}>
                            <div className="absolute inset-0 z-0" style={{ transform: 'scale(0.65)', transformOrigin: 'top left', width: '153.8%', height: '153.8%' }}>
                               {/* Mock Image of Dashboard since rendering the real one causes context issues */}
                               <div className="w-full h-full flex flex-col p-4 border-2 border-transparent" style={{ backgroundColor: '#121214' }}>
                                  <div className="h-[40px] border-b border-white/10 mb-4 rounded-t-lg" style={{ backgroundColor: '#1a1c23' }}></div>
                                  <div className="flex-1 flex gap-4">
                                     <div className="w-[80px] rounded-lg" style={{ backgroundColor: '#1a1c23' }}></div>
                                     <div className="flex-1 rounded-lg" style={{ backgroundColor: '#1a1c23' }}></div>
                                     <div className="flex-1 rounded-lg" style={{ backgroundColor: '#1a1c23' }}></div>
                                     <div className="flex-1 rounded-lg" style={{ backgroundColor: '#1a1c23' }}></div>
                                  </div>
                               </div>
                            ...
                            <div className="absolute top-[96px] left-[0px] w-[81px] h-[310px] z-20 pointer-events-none overflow-hidden">
                               <div className="absolute left-0 right-0 border-y-[1.5px] border-[#47CEAC] shadow-[0_0_15px_rgba(71,206,172,0.15)] z-[100]" style={{ backgroundColor: 'rgba(71,206,172,0.2)', animation: 'histLeftFilterDrag 10s infinite' }}>
                                  <div className="absolute top-[-18px] left-0 right-0 h-[18px] flex items-center justify-center shadow-lg" style={{ backgroundColor: '#FFC107' }}>
                                     <span className="text-[8px] font-black text-black">16 MAR</span>
                                  </div>
                                  <div className="absolute bottom-[-18px] left-0 right-0 h-[18px] flex items-center justify-center shadow-lg" style={{ backgroundColor: '#FFC107' }}>
                ...
                <div style={{ height: chartHeight14 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={torqueDataReal} layout="vertical" margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                      ...
                      <Bar dataKey="value" fill="url(#torqueGradient2)" barSize={25} radius={[0,2,2,0]} isAnimationActive={false} />
      ...
      {showHistoryModal && (
        <div className="fixed left-0 right-0 bottom-0 z-[9999] flex p-3 select-none" style={{ top: '50px', background: '#32343E' }}>
          <style>{`@keyframes expandCard2{from{opacity:0;transform:scale(0.95)}to{opacity:1;transform:scale(1)}}`}</style>
          <div className="flex-1 flex flex-col overflow-hidden text-white" style={{ background:'#1F1F1F', border:'1px solid #525252', borderRadius:'6px', boxShadow:'0 4px 6px rgba(0,0,0,0.3)', animation:'expandCard2 0.22s cubic-bezier(0.2,0,0,1)', transformOrigin:'center center' }}>
...
export default VisualizacionPro2;
