...
import { Settings, AlertTriangle, Bell, Check, X, History, Printer, ChevronLeft, ChevronRight, ChevronDown, Lock } from 'lucide-react';
...
const VisualizacionMedium: React.FC<DashboardProps> = ({ data, data2h, allData, latestPoint, loading, timeRange, onTimeRangeChange, isHistorical }) => {
  ...
  const microDemoStyles = `
    @keyframes kpiCursor {
      ...
      20% { transform: translate(180px, 55px); opacity: 1; }
      40% { transform: translate(180px, 55px); opacity: 1; }
      50% { transform: translate(150px, 100px); opacity: 1; }
      55% { transform: translate(150px, 100px) scale(0.8); opacity: 1; }
    ...
    @keyframes tqCursor {
      ...
      15% { transform: translate(180px, 90px); opacity: 1; }
      30% { transform: translate(180px, 90px); opacity: 1; }
      40% { transform: translate(180px, 115px); opacity: 1; }
      50% { transform: translate(180px, 115px); opacity: 1; }
      60% { transform: translate(150px, 145px); opacity: 1; }
      65% { transform: translate(150px, 145px) scale(0.8); opacity: 1; }
    ...
    @keyframes alCursor {
      ...
      40% { transform: translate(255px, 68px); opacity: 1; }
      48% { transform: translate(255px, 68px) scale(0.8); opacity: 1; }
    ...
    <div className="flex flex-col gap-2 h-full w-full overflow-hidden select-none">
              ...
              <div className="absolute top-full left-0 mt-2 w-[290px] glass-tooltip rounded-xl p-3.5 z-[200] pointer-events-none flex flex-col gap-3 origin-top-left animate-in fade-in zoom-in duration-200">
                <div className="flex items-center justify-between border-b border-white/10 pb-2">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-[#47CEAC] animate-pulse" />
                ...
                <p className="text-[10px] text-white/70 leading-snug">El proceso completo: configura el KPI y mira cómo el gráfico reacciona en tiempo real.</p>
                <div className="relative w-full bg-[#1A1C23] rounded-lg border border-white/10 overflow-hidden flex flex-col gap-2 p-3">
                  <div className="demo-cursor" style={{ animation: 'kpiCursor 6s ease-in-out infinite' }} />
                  <div className="bg-[#1F1F1F] rounded-lg p-3 w-[160px] mx-auto shadow-2xl border border-white/10 flex flex-col relative z-20">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <div className="w-full ml-1 p-1 text-[13px] text-black h-7 bg-white font-bold flex items-center border-[1.5px] border-transparent shadow-[0_0_0_1.5px_#3b82f6] demo-kpi-type" />
                    </div>
                  ...
                  <div className="relative w-full h-[80px] bg-[#111] rounded border border-white/5 overflow-hidden flex items-end px-2 gap-1.5 pt-4 mt-1">
                    {[40, 60, 35, 80, 50, 70, 45, 65, 85].map((h, i) => (
                      <div key={i} className="flex-1 bg-gradient-to-t from-[#8b5cf6]/20 to-[#8b5cf6]/80 rounded-t-sm" style={{ height: `${h}%` }} />
                    ))}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={currentBarData8} margin={{ top: 4, right: 2, left: 0, bottom: 16 }}>
                  ...
                  <Bar dataKey="val" radius={[1,1,0,0]} maxBarSize={12}>
                    {currentBarData8.map((_,i) => <Cell key={i} fill={chartType8 === 'juntas' ? '#8b5cf6' : (i%2===0 ? 'url(#gradientSencilloM)' : 'url(#gradientDobleM)')} />)}
                  </Bar>
          ...
          <div className="grid grid-cols-[1.4fr_1fr] gap-2 h-[220px] shrink-0">
            <div className="glass-panel p-3 flex flex-col relative overflow-visible group hover:z-[100]">
              ...
              >
                ...
                >
                  ...
                  {showAlarmPreview && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[340px] glass-tooltip rounded-xl p-4 z-[200] flex flex-col gap-3 pointer-events-none cursor-default origin-top animate-in fade-in zoom-in duration-200">
                      ...
                      <p className="text-[10px] text-white/70 leading-snug">Monitorea notificaciones en tiempo real. Reconoce alarmas al instante de gases, torques o ciclos.</p>
                      <div className="relative w-full bg-[#1A1C23] rounded-lg border border-white/10 overflow-hidden flex flex-col gap-2 p-3 min-h-[140px]">
                        <div className="demo-cursor" style={{ animation: 'alCursor 6s ease-in-out infinite' }} />
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-2 bg-[#2B2D38] border border-white/5 rounded-md px-2 py-2">
                            ...
                            <div className="w-6 h-6 rounded bg-white/5 flex items-center justify-center">
                              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-white/40"><polyline points="20 6 9 17 4 12" /></svg>
                            </div>
                          </div>
                ...
                <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest text-center px-4">Módulo de Alarmas</span>
            ...
            >
              ...
              {showTorquePreview && (
                <div className="absolute bottom-0 right-full mr-2 w-[290px] glass-tooltip rounded-xl p-3.5 z-[200] pointer-events-none flex flex-col gap-3 origin-bottom-right animate-in fade-in zoom-in duration-200">
                  ...
                  <p className="text-[10px] text-white/70 leading-snug">Modifica los límites MIN/MAX y observa cómo la ventana operativa se actualiza.</p>
                  <div className="relative w-full bg-[#1A1C23] rounded-lg border border-white/10 overflow-hidden flex flex-col gap-2 p-3">
                    ...
                    <div className="bg-[#525252] rounded-lg p-3 w-[170px] mx-auto shadow-2xl border border-white/5 flex flex-col relative z-20">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div className="w-full p-1 text-[11px] text-black h-6 bg-white font-bold flex items-center shadow-[0_0_0_1.5px_#3b82f6] demo-tq-typemin" />
                      </div>
                      <div className="flex items-center gap-2 mb-3">
                        <span className="text-[9px] text-white">MAX</span>
                        <div className="w-full p-1 text-[11px] text-black h-6 bg-white font-bold flex items-center shadow-[0_0_0_1.5px_#3b82f6] demo-tq-typemax" />
                      </div>
                ...
                <div style={{ height: chartHeight14 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={torqueDataReal} layout="vertical" margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                      ...
                      <Bar dataKey="value" fill="url(#torqueGradientM)" barSize={25} radius={[0,2,2,0]} isAnimationActive={false} />
      ...
      {showHistoryModal && (
        <div className="fixed left-0 right-0 bottom-0 z-[9999] flex p-3 select-none" style={{ top: '50px', background: '#32343E' }}>
          <style>{`@keyframes expandCardM{from{opacity:0;transform:scale(0.95)}to{opacity:1;transform:scale(1)}}`}</style>
          <div className="flex-1 flex flex-col overflow-hidden text-white" style={{ background:'#1F1F1F', border:'1px solid #525252', borderRadius:'6px', boxShadow:'0 4px 6px rgba(0,0,0,0.3)', animation:'expandCardM 0.22s cubic-bezier(0.2,0,0,1)', transformOrigin:'center center' }}>
...
export default VisualizacionMedium;
